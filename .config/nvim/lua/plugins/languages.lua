return {
  { -- LSP Configuration & Plugins
    ...
    config = function()
      ...
      -- LSP servers and clients communicate which features they support. Neovim
      -- doesn't advertise everything in the spec by default; blink.cmp extends the
      -- client capabilities (snippet support, completion-item resolve, etc.), which
      -- we broadcast to every server below.
      ...
      -- clangd is excluded from auto-install: the system clangd (/usr/bin/clangd)
      -- is used instead, so mason doesn't install it on startup in unrelated repos.
      -- Everything else in `servers` is installed (names translated to mason
      -- packages by mason-lspconfig).
      ...
      -- Apply the capabilities globally, then register and enable each server.
  ...
  { -- Autocompletion
    ...
    dependencies = {
      ...
      -- Lets blink consume the `conjure` completion source (wired under `sources`).
    ...
    opts = {
      ...
      keymap = {
        ...
        -- Accept the completion (auto-imports / expands snippets if the LSP sent one).
        ['<CR>'] = { 'accept', 'fallback' },

        -- Manually trigger / toggle the docs.
        ['<C-Space>'] = { 'show', 'show_documentation', 'hide_documentation' },

        -- <C-l> moves to the right of each snippet expansion stop; <C-h> moves back.
      ...
      sources = {
        ...
        -- Add the conjure REPL source in Lisp buffers (via blink.compat).
        per_filetype = {
          scheme = { 'conjure', 'lsp', 'path', 'snippets' },
          racket = { 'conjure', 'lsp', 'path', 'snippets' },
          guile = { 'conjure', 'lsp', 'path', 'snippets' },
        },
        providers = {
  ...
  },
  {
    ...
    -- Registers the `conjure` completion source, which blink consumes through
    -- blink.compat (wired in the blink.cmp spec). blink.compat must load first
    -- so cmp-conjure's `require('cmp')` resolves to the compat shim.
    ft = { 'racket', 'guile', 'scheme' },
